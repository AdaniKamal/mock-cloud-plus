...
const App = () => {
  ...
  const [timeLeft, setTimeLeft] = useState(70 * 60); // set to 90 * 60 if you want 1h30
  const [darkMode, setDarkMode] = useState(false);

  // NEW: sidebar + content selections
  ...
  const startExam = () => {
    ...
    setSubmitted(false);
    setCurrentIndex(0);
    setTimeLeft(70 * 60); // change to 90 * 60 if you want 1h30
    setView('exam');
  ...
  const Sidebar = () => (
    <>
      <div className={`cp-sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <div className="mb-3">
          <button className="btn btn-outline-primary w-100" onClick={openNotes}>
            Notes
          </button>
        </div>
  ...
  const renderHome = () => (
    <div className="container min-vh-100 d-flex flex-column justify-content-center align-items-center text-center py-4">
      ...
      {/* Sidebar is usable from Home too */}
      <Sidebar />
    </div>
  );
      ...
      <>
        ...
        <div className="container py-4">
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
            ...
            <button className="btn btn-outline-secondary" onClick={() => setSidebarOpen(s => !s)}>☰ Resources</button>
          </div>
              ...
              {sim.instructions && (
                <div className="mb-3">
                  {Array.isArray(sim.instructions) ? (
                    <ol>
                      {sim.instructions.map((step, i) => <li key={i}>{step}</li>)}
                    </ol>
                  ) : (
